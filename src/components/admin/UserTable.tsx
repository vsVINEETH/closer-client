...
import { ChevronsUpDown, SlidersHorizontal, Search } from 'lucide-react';
...
const UserTable: React.FC = () => {
    ...
    useEffect(() => {
       setResult(
          searchValue ? filterData.filter(employee =>
    ...
    const handleFilter = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const filtered = userData.filter((user) => {
            ...
            const isWithinDateRange = 
                (!startClone || userDate >= startClone) &&
                (!endClone || userDate <= endClone);
    
            // Check if employee status matches
            const isStatusMatch = status === undefined || user.isBlocked === status;
...
//pagination
    ...
    //ban
    const handleBanUser = async(id: string, duration: string) => {
        const response = await  handleRequest({
        ...
        <table className="w-full mt-4 text-left table-auto min-w-max">
            ...
            <th
                ...
                <p
                    className="flex items-center justify-between gap-2 font-sans text-sm antialiased font-normal leading-none text-blue-gray-900 opacity-70">
                    User
                    <ChevronsUpDown size={13} onClick={() => handleSort('username')}/>
                </p>
                </th>
                <th
                className="p-4 transition-colors cursor-pointer border-y border-blue-gray-100 bg-blue-gray-50/50 hover:bg-blue-gray-50">
            ...
            <td className="p-4">
                ...
                <td className="p-4">
                <p className="block font-sans text-sm antialiased font-normal leading-normal text-blue-gray-900">
                    {value.reportedUsers.length ? value.reportedUsers.length : 0}
                </p>
                </td>
                <td className="p-4">
                {!value.isBanned && (
                <select
                    onChange={(e) => handleBanUser(value.id, e.target.value)}
                    className="text-xs p-1 border border-gray-300 rounded-md bg-white shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-400"
                >
                ...
                {value.isBanned && (
                    ...
                    onClick={() => handleUnban(value.id)}
                    >
                        Unban
                    </button>
                    <p className="text-xs font-medium text-gray-700 dark:text-lightGray">
                        {Math.ceil(
                        (new Date(value.banExpiresAt).getTime() - new Date().getTime()) /
                            (1000 * 60 * 60 * 24)
                        )}{" "}
                        days left
                    </p>
